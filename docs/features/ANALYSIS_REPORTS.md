...
分析レポートはユーザーIDに紐づくため、アカウント移行時の扱いを以下の方針で統一する。

- ゲストからGoogleへのアップグレードは、同一ユーザーIDでの移行を前提とする。
- アップグレード処理中にユーザーIDが別アカウントへ切り替わった場合は、アップグレードを失敗扱いにする。
- 失敗時は元のゲストセッションを復元し、以下を案内する。  
  `既存のGoogleアカウントがあるためアップグレードに失敗しました。ログインから入り直してください。`
- 新規アカウント登録を選んだ場合、ゲストデータ（分析レポート含む）は引き継がれない。
- 既存Googleアカウントで利用していたレポートは、そのGoogleアカウント側に残る（上書きしない）。
