...
### 1. Blobビジュアライザーの品質改善
- **現状**: 自前実装の`BlobVisualizerView`は機能するが、視覚的な品質が低い
- **計画**: Lottieなどのアニメーションフレームワークを導入予定
- **優先度**: 中

### 2. 長時間録音の制限機能がない
- **現状**: 録音時間の上限が設定されていないため、ユーザーが誤って長時間録音を続ける可能性がある
- **リスク**:
  - ストレージ容量の圧迫
  - バッテリー消費
  - 不要な大容量データのアップロード
- **計画**: 30分の録音時間上限を設定し、超過時は自動停止または警告表示
- **優先度**: 高

### 3. ネットワーク障害時のエラーハンドリング不足
- **現状**: 新しい`FullScreenRecordingView`では、Wi-Fi/モバイルデータ通信の不安定性による送信失敗時の処理が不十分
- **問題点**:
  - アップロード失敗時の再試行ロジックがない
  - ユーザーへのフィードバックが不明確
  - 失敗した録音データの保存・再送信機能がない
- **計画**:
  - リトライ機構の実装
  - オフライン時の録音データ一時保存
  - ネットワーク復旧時の自動再送信
