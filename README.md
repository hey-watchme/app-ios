...
| **高** | **UGC（コメント）対策機能** | コメント機能に、不適切なコンテンツをユーザーが**通報する機能**や、特定のユーザーを**ブロックする機能**がありません。これもリジェクトの可能性が非常に高い項目です。 | 1.2 | ✅ **実装済み** |
...
- **お問い合わせ・通報機能**: アプリ内からのお問い合わせ、不適切なコメントの通報（2025-10-18追加）
...
## 📮 お問い合わせ・通報機能（2025-10-18追加）

### 概要

App Store審査要件（ガイドライン1.2：UGC対策）に対応するため、お問い合わせ・通報機能を実装しました。

### 機能詳細

#### 1. **一般的なお問い合わせ**
- **アクセス方法**: マイページ → アカウント設定 → お問い合わせ
- **用途**:
  - お問い合わせ
  - ご意見・ご要望
  - バグの報告
...
#### 2. **コメント通報**
- **アクセス方法**: ダッシュボードのコメント表示部分 → 「通報」ボタン
- **用途**: 不適切なコンテンツの通報
- **特徴**:
  - 自分のコメントには「削除」ボタンが表示
  - 他人のコメントには「通報」ボタンが表示
  - 通報時には対象コメントの内容が自動的に表示される
...
- category (TEXT): inquiry, feedback, bug_report, report_content, other
- message_body (TEXT): メッセージ本文
- context_type (TEXT): general, comment, user, bug
- target_comment_id (UUID): 通報対象のコメントID
- app_version (TEXT): 自動付加
- os_version (TEXT): 自動付加
- device_model (TEXT): 自動付加
...
- **Models/Message.swift**: データモデル定義
- **FeedbackFormView.swift**: 共通フォームUI
- **SupabaseDataManager.swift**: `submitFeedback()`メソッド
- **AccountSettingsView.swift**: お問い合わせボタン
- **SimpleDashboardView.swift**: コメント通報ボタン

### App Store審査対応状況

✅ **UGC対策機能（ガイドライン1.2）**: 実装済み
- ユーザーが不適切なコンテンツを通報できる
- 各コメントに通報ボタンを配置
- 通報内容はデータベースに保存され、運営側で確認可能
