...
### v9.33.0 (2025-10-07)
- **パフォーマンス最適化**: アプリ起動時間の調査と改善
  - Supabaseクライアントを遅延初期化に変更（Singleton + lazy）
  - 詳細なパフォーマンスログを追加（各処理の所要時間を計測）
  - 起動フローの可視化（ボトルネック特定のための基盤整備）
- **調査結果**: 実機で5秒（業界標準レベル）、アプリコード自体は0.05秒で最適化済み

### v9.32.0 (2025-10-04)
- **デバイス未選択時のUI改善**: オンボーディング体験の向上
  - 「あなたの声から「こころ」をチェックしよう。」メッセージを表示
  - 3つのアクションボタンを配置：
    1. このデバイスで測定する（新規デバイス登録）
    2. サンプルを見る（サンプルデバイス選択）
    3. QRコードでデバイスを追加
  - ContentView.swift と DeviceSelectionView.swift に実装
- **サンプルデバイス機能の追加**:
  - 全ユーザー共通のサンプルデバイス（device_id: a1b2c3d4-e5f6-4a5b-8c9d-0e1f2a3b4c5d）
  - デバイス設定画面に「サンプルデバイス」セクションを追加
  - データベースから動的に取得（device_type: observer）
  - observerデバイスのため録音ボタン（FAB）は非表示
  - DeviceManager.sampleDeviceID 定数で管理

### v9.31.0 (2025-10-04)
- **メール確認の無効化**: ユーザー体験最適化のため、会員登録時のメール確認を廃止
- **自動ログイン機能**: サインアップ成功後、メール確認状態に関係なく自動的にログイン
  - `UserAccountManager.performSignIn()` メソッドを追加（async/await対応）
  - `signUp()` メソッドから自動的に `performSignIn()` を呼び出し
- **画面遷移の改善**:
  - SignUpView/LoginViewでの `isAuthenticated` 変化の検知を強化
  - 会員登録後、即座にメイン画面へ遷移
  - SignUpSuccessView（メール案内画面）の表示を無効化
- **Supabase設定**: Authentication → Providers → Email → Confirm email を OFF に変更
- **UX向上**: 「登録→即利用開始」のシームレスな体験を実現

### v9.30.0 (2025-10-02)
- サインアップ機能とユーザー管理機能を追加
- SignUpViewとSignUpSuccessViewを実装
- UserAccountManagerにサインアップ、ログアウト、アカウント削除機能を追加
